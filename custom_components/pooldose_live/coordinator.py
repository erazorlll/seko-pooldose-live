...
class PooldoseLiveCoordinator(DataUpdateCoordinator[dict[str, ResolvedChannel]]):
    ...
    def _handle_event(self, event: TransportEvent) -> None:
        if event.kind == "snapshot":
            ...
            if self.is_stale:
                self.is_stale = False
                self.async_update_listeners()
        elif event.kind in ("connected", "disconnected", "watchdog"):
            _LOGGER.debug(
    ...
    def _handle_snapshot(self, event: TransportEvent) -> None:
        ...
        # Resolution-Schwellen und Heartbeat (Konzept §5.6) ist P3-Scope -
        # das hier verhindert nur den trivialen Fall "kompletter Zyklus ohne
        # jede Änderung", der laut §8.2 die deutliche Mehrheit der Zyklen ist.
